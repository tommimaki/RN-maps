...
export default function App() {
  ...
  const fetchRestaurants = (lat, lng) => {
    fetch(
      `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=1500&type=restaurant&key=${API_KEY2}`
    )
      .then((response) => {
  ...
  return (
    <View style={styles.container}>
      <MapView style={styles.map} initialRegion={initial} region={region}>
        <Marker coordinate={coordinates} title={osoite} />
